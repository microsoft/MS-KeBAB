...
@pytest.fixture
def rebel_single_record_input_file_path() -> Path:
...
def test_run(rebel_sample_dir_input_file_path: Path, tmp_path: Path) -> None:
    """Test the extraction of entities and properties from a single REBEL JSON document record."""
    extractor = RebelFragmentExtractor(rebel_dir=rebel_sample_dir_input_file_path, output_dir=tmp_path)
    extractor.run()
